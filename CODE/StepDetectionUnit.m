...
%% NOTE
% script for test of unit
clear all 
close all

%% INIZIALIZZAZIONI

load('DataRaw\85ppm\ExpGusma_Session6_Number3_Calibrated_SD.mat')
%load('DataRaw\100ppm\ExpGusma_Session7_Number3_Calibrated_SD.mat')
...
% load('DataRaw\ESPERIMENTO GIAPPONE\WalkInertialEMG.mat')
% soggetto=1;
% stringa="F2";
% i=1;
% [accDataRaw , gyroDataRaw]= acquisizione_dati(WalkInertial,soggetto,stringa,i);

% load('DataRaw\BASELINE\2020-12-07_22.49.42_ExpGusma_MultiSession\ExpGusma_Session1_Number3_Calibrated_SD.MAT')
% accDataRawInit=[Number3_Accel_LN_X_CAL Number3_Accel_LN_Y_CAL Number3_Accel_LN_Z_CAL];
% gyroDataRawInit=[Number3_Gyro_X_CAL Number3_Gyro_Y_CAL Number3_Gyro_Z_CAL];
% X=LinearKalmanFilter(accDataRaw,gyroDataRaw,accDataRawInit,gyroDataRawInit);
...
function printStepDetection(signal1,signal2,signal3,signal4,endStanceFase,startStanceFase,t,Title,Xax,Yax,indexFigure)
    ...
    ylabel(Yax)
    title(Title)
    legend('Magnitude','Variance','B2','B1')
end
